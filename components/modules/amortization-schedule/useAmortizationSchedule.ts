...
import type { OffsetsInput, OverviewRange, PaymentStatus, TableSort } from "./schema";
...
export const amortizationKeys = {
  ...
  schedule: (caseId: string) => [QUERY_SCOPES.amortization, "schedule", caseId] as const,
  overview: (caseId: string, range: OverviewRange) => [QUERY_SCOPES.amortization, "overview", caseId, range] as const,
  table: (caseId: string, page: number, size: number, status?: PaymentStatus, sort?: TableSort) =>
    [QUERY_SCOPES.amortization, "table", caseId, page, size, status ?? "all", sort ?? "default"] as const,
};
...
/**
 * The case's schedule state (latest version + trades freshness) — the server
 * truth the échéancier panel derives its workflow from. A 404 (nothing imported
 * yet) resolves to `null`, matching the overview's semantics.
 */
export function useLatestSchedule(caseId: string) {
  return useQuery({
    queryKey: amortizationKeys.schedule(caseId),
    queryFn: () => getLatestSchedule(caseId).catch(() => null),
  });
}
...
export function useAmortizationTable(
  ...
  return useQuery({
    queryKey: amortizationKeys.table(caseId, params.page, params.size, params.status, params.sort),
